package framework.resources
{
	...
	public class LocalResGetter extends AResGetter implements IEventListener
	{
		...
		protected function LocalResGetter_2(path:String):void
		{
			init(path);
		}
		
		public function LocalResGetter()
		{
		 ...
		 * @return
		 */
		override public function getBinary(name:String):ByteArray
		{
		 ...
		 * @return
		 */
		override public function getJson(name:String):Object{
			var obj:Object= null;
		 ...
		 * @return
		 */
		override public function getSheet(name:String):SpriteSheet{
			...
			if(_dataTable[name] == null)
			{
				...
				ss = new SpriteSheet();
				ss.SpriteSheet_2(jsonObj, item.subkeys.split(','), sheet);
				_dataTable[name] = ss ;
			}
			else
			{
		 ...
		 * @return
		 */
		override public function getFontSheet(name:String):FontSheet{
		...
		public function getAudio(name:String, type:String):Player{
			...
			{
				var stream:InputStream= FileIO.getInputStream(item.url());
		 ...
		 * @return
		 */
		override public function getUrl(name:String):String{
			var url:String;
		 ...
		 * @param name 图片资源的名称
		 */
		override public function release(name:String):void{
			if(_dataTable[name] != undefined)
			{
